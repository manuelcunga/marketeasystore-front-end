...
export class OrdersService {
  ...
  ListALlOrders(): Observable<OrderResponse[]> {
    ...
    return this.httpClient.get<OrderResponse[]>(`${this.apiUrl}/orders`, { headers: httpOption, }
    ).pipe(
      map((obj)=> obj),
  ...
  DeleteOrder(id: string): Observable<OrderResponse[]> {
    ...
    return this.httpClient.delete<OrderResponse[]>(`${this.apiUrl}/order/${id}`, { headers: httpOption, }
    ).pipe(
      map((obj)=> obj),
      catchError(e => this.handleError(e))
    )
